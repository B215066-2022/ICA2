...
RunLoopXa = True # Used in the "ASK USER TO CHOOSE BETWEEN INPUT TAXONID AND SEARCH NCBI DATABASE" section 
RunLoopXb = True # Used in the "ASK USER TO INPUT A PHYLUM" section 
RunLoopXc = True # Used in the "ASK USER TO INPUT TAXONID AND PROTEIN FAMILY" section
RunLoopXd = True # Used in the "ASK USER TO LIST ALL FASTA PROTEIN SEQUENCES" section
RunLoopXe = True # Used in the "ASK USER TO VIEW CHOSEN PROTEIN SEQUENCE ACCESSION" section
RunLoopXf = True # Used in the "ASK USER TO VIEW RESULT OF BLAST ALIGNMENT" section
RunLoopXg = True # Not used by any
RunLoopXh = True # Used in the "ASK USER TO VIEW RESULT OF CLUSTALO MSA" section
RunLoopXi = True # Used in the "ASK USER TO SELECT FURTHER PROTEIN ANALYSIS" section
...
time.sleep(1)
print("If you know the taxonomy ID for the organism species you are interested in, you can specify it.")
time.sleep(1)
print("Alternatively, you can search through the NCBI database for the TaxonID.")
time.sleep(1)

# ASK USER TO CHOOSE BETWEEN INPUT TAXONID AND SEARCH NCBI DATABASE
while RunLoopXa:
    print("How would you proceed?\n1) Proceed with TaxonID\n2) Search NCBI database")
    time.sleep(1)
    ...
    if QueryU == '1':
        ...
        while RunLoopXb:
            ...
            if QueryV.lower() in OptionX:
                ...
                time.sleep(1) # df = pd.read_csv('{QueryV}.tsv', sep="\t", na_values=['-'])
...
while RunLoopXc:
    ...
    time.sleep(1)
    print("For example: For the mosquito family Cullinicea, input the TaxonID 7157 or 4890 for the fungi family Ascomycete.")
    time.sleep(1)     
    QueryA = input('Please input the TaxonID of your organism now: ')
    print("------------------------------")
    print("Now, I need you to specify the family of protein you are interested in.")
    time.sleep(1)
    print("For example: pyruvate dehydrogenase or trehalose-6-phosphate synthase.")
    time.sleep(1)
    ...
    if QueryX.lower() in OptionA:
        ...
        if UIDcount < 1:
            ...
            time.sleep(1)
            print("Please consider a different TaxonID or protein family.")
            time.sleep(1)
            continue
        ...
        # ASK USER TO LIST ALL FASTA PROTEIN SEQUENCES
        while RunLoopXd:
            ...
            if QueryC.lower() in OptionA:
                ...
                print("We can view the the protein sequences later.")
                break
        ...
        # ASK USER TO VIEW RESULT OF BLAST ALIGNMENT
        while RunLoopXf:
        ...
        # ASK USER TO VIEW RESULT OF CLUSTALO ALIGNMENT
        while RunLoopXh:
        ...
        # We should make this run in a separate python script later
        print("------------------------------")
        ...
        while RunLoopXi:
            ...
            if QueryI == '1': # For "Plot interactive multiple sequence alignment” with EMBOSS showalign
            ...
            elif QueryI == '2': # For “Analyse consensus in multiple sequence alignment” with EMBOSS infoalign
                time.sleep(1)
                ...
                time.sleep(3)
                # EMBOSS infoalign
                subprocess.call(f"infoalign \"ICA2/{QueryBr}/{QueryBr}_Aligned.msf\" -nousa -outfile \"ICA2/{QueryBr}/{QueryBr}_InfoAlign.infoalign\" ",shell=True)
# Take msf alignment file for showalign and generate output in infoalign to be shown on screen
...
# Take msf alignment file for showalign and generate output in html to be used later
                print("Analysis complete!")
                time.sleep(1)
                TempFileH = open(f"ICA2/{QueryBr}/{QueryBr}_InfoAlign.infoalign").read()
                time.sleep(1)
